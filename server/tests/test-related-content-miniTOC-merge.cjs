...
(async () => {
  ...
  // Find heading 3 'Related Content'
    // Find toggle with title 'Related Content'
    const toggleIndex = blocks.findIndex(b => b.type === 'toggle' && ((b.toggle || {}).rich_text || []).map(rt => rt.text.content).join('').trim().toLowerCase() === 'related content');
    assert(toggleIndex >= 0, 'Missing Related Content toggle block');

    // Ensure that the toggle's children include bulleted list items + paragraphs or link descriptions
    const toggleBlock = blocks[toggleIndex];
    const afterChildren = (toggleBlock.toggle && toggleBlock.toggle.children) || [];
    const bulletCount = afterChildren.filter(b => b.type === 'bulleted_list_item').length;
    const paraCount = afterChildren.filter(b => b.type === 'paragraph').length;
  ...
  const bulletTexts = afterChildren.filter(b => b.type === 'bulleted_list_item').map(b => (b.bulleted_list_item || {}).rich_text.map(rt => rt.text.content).join('').trim());
