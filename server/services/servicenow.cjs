...
async function extractContentFromHtml(html) {
  ...
  // Remove table export dropdown buttons and filter divs (must be global - can appear in list/paragraph content)
  html = html.replace(/<div[^>]*class="[^\"]*zDocsFilterTableDiv[^\"]*"[^>]*>[\s\S]*?<\/div>/gi, "");
  html = html.replace(/<div[^>]*class="[^\"]*smartTable[^\"]*"[^>]*>[\s\S]*?<\/div>/gi, "");
