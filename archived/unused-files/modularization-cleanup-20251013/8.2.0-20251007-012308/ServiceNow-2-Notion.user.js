...
(function () {
  ...
  async function findAndClickNextButton(
    ...
    // Try to find the button with retries (5 seconds between attempts)
    while (!nextButton && findAttempts < maxFindAttempts) {
      ...
      if (!nextButton && findAttempts < maxFindAttempts) {
        debug(`⚠️ Next page button not found, waiting 5s before retry...`);
        showToast(
          `Next button not found, retrying in 5s (attempt ${findAttempts}/${maxFindAttempts})...`,
          4000
        );
        await new Promise((resolve) => setTimeout(resolve, 5000));
      }
    ...
    if (!nextButton) {
      debug(`❌ Next page button not found after ${maxFindAttempts} attempts`);
      showToast(
        `❌ Next page button could not be found after ${maxFindAttempts} attempts`,
        5000
      );
    }
